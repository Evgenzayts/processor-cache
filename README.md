...
```cpp
int* _arr = new int[32]; // 4 * 32 = 128 байт
int i = _arr[0]; // чтение первого элемента (первых четырёх байт) из массива _arr
```
...
```cpp
int j = _arr[0]; // чтение с 0 по 3 байт
int k = _arr[4]; // чтение с 16 по 19 байт
int m = _arr[15]; // чтение с 60 по 63 байт
```
...
Теперь массив `_arr` полностью находится в кэше `L1`, следовательно любое обращение к элементам массива `_arr` будет приводить к попаданию в кэш `L1` и иметь очень высокую скорость чтения.
